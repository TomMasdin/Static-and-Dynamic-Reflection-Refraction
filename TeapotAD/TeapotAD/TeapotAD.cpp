...
//To cycle from set rotations in the camera
int currentRotation;
...
static void key_callback(GLFWwindow* window, int key, int cancode, int action, int mods)
{
	...
	if (key == '1' && action == GLFW_RELEASE)
		if (scene)
			scene->switchShader(!(scene->switchingShader()));
	if (key == 'T' && action == GLFW_RELEASE)
	{
		...
		camera.switchToFace(currentRotation);
		currentRotation++;
	}
	
	if (scene)
		scene->keyHandler(&camera, key);
}
...
{ 
	...
	//	std::cout <<"deltaX " << deltaX << " deltaY " << deltaY << "\n";
		...
		camera.rotate((deltaX*ROTATE_VELOCITY) * -1.f, (deltaY*ROTATE_VELOCITY) * -1.f);
	...
	if (glfwGetMouseButton(window,GLFW_MOUSE_BUTTON_RIGHT) )
	{
		...
		camera.pan((deltaX*MOVE_VELOCITY) * -1.f, (deltaY*MOVE_VELOCITY) * -1.f);
	...
	if (glfwGetMouseButton(window,GLFW_MOUSE_BUTTON_MIDDLE) )
	{
		...
		camera.roll((deltaX*ROTATE_VELOCITY) * -1.f);
